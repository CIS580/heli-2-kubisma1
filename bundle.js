...
},{"./bullet-pool":2,"./game":3,"./vector":4}],2:[function(require,module,exports){
module.exports = exports = BulletPool;

function BulletPool(maxSize) {
  // We'll be storing 4 information
  // 4 cells for each bullet
...
BulletPool.prototype.update = function(elapsedTime, callback) {
  for(var i = 0; i < this.end; ) {
    this.pool[4*i] += this.pool[4*i+2];
    this.pool[4*i+1] += this.pool[4*i+3];

    if(callback({x: this.pool[4*i], y: this.pool[4*i+1]})) { // bullet is dead
